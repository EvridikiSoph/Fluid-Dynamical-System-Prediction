...
predict.narx = function (model, Y, U, K = 1, bw) {
  cat('Running narx prediction ... \n')
  prediction = predict.default(model, Y, U, K, bw)
  return(prediction)
}

# Prediction default function
predict.default = function (model, Y, U, K = 1, bw) {
...
# ignore K
# Output: list containing predictions for each time series
freeRun = function (model, Y, U, K, bw) {
