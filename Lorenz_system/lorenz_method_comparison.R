...
for (j in 1:nb_series) {
  df_joined = data.frame(c(narx_sd_nn[j], narx_sd_n005[j], narx_sd_n01[j], narx_sd_n02[j]),
                         c(narmax_sd_nn[j], narmax_sd_n005[j], narmax_sd_n01[j], narmax_sd_n02[j]),
                         c(sindy_sd_nn[j], sindy_sd_n005[j], sindy_sd_n01[j], sindy_sd_n02[j]))

  colnames(df_joined) = pred_methods
  
  NRMSEs_x_long = gather(df_joined, key="Method", value="NRMSE", 1:3)
  NRMSEs_x_long$Noise = c('0', '0.005', '0.01', '0.02')

  print(ggplot(NRMSEs_x_long, aes(x = factor(Noise, level = c('0', '0.005', '0.01', '0.02')), y = NRMSE, group = Method)) +
          theme_minimal() +
...
# Compare means
for (j in 1:nb_series) {
  df_joined = data.frame(c(narx_means_nn[j], narx_means_n005[j], narx_means_n01[j], narx_means_n02[j]),
                         c(narmax_means_nn[j], narmax_means_n005[j], narmax_means_n01[j], narmax_means_n02[j]),
                         c(sindy_means_nn[j], sindy_means_n005[j], sindy_means_n01[j], sindy_means_n02[j]))
